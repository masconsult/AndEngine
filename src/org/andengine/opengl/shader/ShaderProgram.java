...
public class ShaderProgram implements ShaderProgramConstants {
    ...
    public ShaderProgram(final String pVertexShaderSource, final String pFragmentShaderSource) {
        this(new StringShaderSource(pVertexShaderSource), new StringShaderSource(
                pFragmentShaderSource));
    }

    public ShaderProgram(final IShaderSource pVertexShaderSource,
    ...
    public int getAttributeLocation(final String pAttributeName) {
        ...
        if (location != null) {
            ...
            throw new ShaderProgramException("Unexpected attribute: '" + pAttributeName
                    + "'. Existing attributes: " + this.mAttributeLocations.toString());
        }
    }
    ...
    public int getUniformLocation(final String pUniformName) {
        ...
        if (location != null) {
            ...
            throw new ShaderProgramException("Unexpected uniform: '" + pUniformName
                    + "'. Existing uniforms: " + this.mUniformLocations.toString());
        }
    }
    ...
    protected void compile(final GLState pGLState) throws ShaderProgramException {
        final int vertexShaderID = ShaderProgram.compileShader(
                this.mVertexShaderSource.getShaderSource(pGLState), GLES20.GL_VERTEX_SHADER);
        final int fragmentShaderID = ShaderProgram.compileShader(
                this.mFragmentShaderSource.getShaderSource(pGLState), GLES20.GL_FRAGMENT_SHADER);
    ...
    protected void link(final GLState pGLState) throws ShaderProgramLinkException {
        ...
        GLES20.glGetProgramiv(this.mProgramID, GLES20.GL_LINK_STATUS,
                ShaderProgram.HARDWAREID_CONTAINER, 0);
    ...
    private static int compileShader(final String pSource, final int pType)
            throws ShaderProgramException {
    ...
    private void initUniformLocations() {
        ...
        GLES20.glGetProgramiv(this.mProgramID, GLES20.GL_ACTIVE_UNIFORMS,
                ShaderProgram.PARAMETERS_CONTAINER, 0);
        final int numUniforms = ShaderProgram.PARAMETERS_CONTAINER[0];

        for (int i = 0; i < numUniforms; i++) {
            GLES20.glGetActiveUniform(this.mProgramID, i, ShaderProgram.NAME_CONTAINER_SIZE,
                    ShaderProgram.LENGTH_CONTAINER, 0, ShaderProgram.SIZE_CONTAINER, 0,
                    ShaderProgram.TYPE_CONTAINER, 0, ShaderProgram.NAME_CONTAINER, 0);
            ...
            // failsafe, location is invalid - probably the glGetActiveUniform()
            // return the invalid length. calculate it manually.
            if (location == -1) {
                length = 0;
                while (length < NAME_CONTAINER_SIZE && ShaderProgram.NAME_CONTAINER[length] != '\0') {
                    length++;
    ...
    private void initAttributeLocations() {
        ...
        ShaderProgram.PARAMETERS_CONTAINER[0] = 0;
        GLES20.glGetProgramiv(this.mProgramID, GLES20.GL_ACTIVE_ATTRIBUTES,
                ShaderProgram.PARAMETERS_CONTAINER, 0);
        final int numAttributes = ShaderProgram.PARAMETERS_CONTAINER[0];

        for (int i = 0; i < numAttributes; i++) {
            GLES20.glGetActiveAttrib(this.mProgramID, i, ShaderProgram.NAME_CONTAINER_SIZE,
                    ShaderProgram.LENGTH_CONTAINER, 0, ShaderProgram.SIZE_CONTAINER, 0,
                    ShaderProgram.TYPE_CONTAINER, 0, ShaderProgram.NAME_CONTAINER, 0);
            int length = ShaderProgram.LENGTH_CONTAINER[0];
            /*
    ...
    public void setUniformOptional(final String pUniformName, final float[] pGLMatrix) {
        ...
        if (location != ShaderProgramConstants.LOCATION_INVALID) {
            GLES20.glUniformMatrix4fv(this.getUniformLocationOptional(pUniformName), 1, false,
                    pGLMatrix, 0);
        }
    }
    ...
    /**
     * @param pUniformName
     * @param pTexture the index of the Texture to use. Similar to
